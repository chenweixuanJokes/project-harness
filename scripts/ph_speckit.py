...
def build_install_plan(repo: Path, staging: Path, contract: dict) -> list[dict]:
    required = [*SKILL_BASELINE_RELS.values(), *SHARED_RUNTIME_RELS, CONSTITUTION_MEMORY_REL]
    conflicts = []
    for rel in required:
        try:
            ensure_safe_write_dest(repo, rel)
        except PHError as exc:
            conflicts.append({"kind": "conflict", "path": rel, "reason": str(exc), "data": None})
    if conflicts:
        return conflicts
    items: list[dict] = []
    for core in SPECKIT_CORE_SKILLS:
    ...
    if actual != set(required):
        raise PHError(f"pinned generation file inventory mismatch: {sorted(actual ^ set(required))}")
    return items
