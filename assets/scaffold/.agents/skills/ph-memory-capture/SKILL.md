...
description: "把用户要求记住的内容写入仓库 `.agents/memory/temporary/`。调用门禁：仅当用户当轮明确点名 ph-memory-capture（如「用 ph-memory-capture 记住…」）并要求使用时才调用；只说“记住”“记一下”“写进记忆”等普通描述、上下文提及或讨论技能名称都不触发。已显式启动的同一流程内，用户回答提问或说“继续”仍按原流程接收反馈与恢复，不要求每轮重复点名，也不触发其他技能。不要把查记忆、归档记忆、普通知识问答、只回顾当前对话、录入/规划/验收/废弃意图、或要求改规范/Wiki 误判为本技能。禁止写入秘密与个人信息。"
---
...
**调用门禁**：仅当用户当轮明确点名本技能（如「用 ph-memory-capture 记住…」）并要求使用时才执行。用户只说“记住 X”“记一下”、上下文里提到记忆、或讨论技能名称本身，都不触发；未点名时不代写记忆文件。
