---
name: "ph-specify"
description: "根据用户描述创建或更新功能规格，明确需求范围、使用场景和验收目标。仅在用户主动明确点名调用本技能时执行；普通任务描述、名称提及或上下文关联不触发，执行结束后不自动串联其他技能。已显式启动后不要求每轮重复点名，不触发其他技能。"
...
The text the user typed after `$ph-specify` in the triggering message **is** the feature description. Assume you always have it available in this conversation even if `$ARGUMENTS` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
3. **Create the spec feature directory**:
   ...
   Specs live under the default `.agents/project-harness/specs/` directory unless the user explicitly provides `SPECIFY_FEATURE_DIRECTORY`.

   **Resolution order for `SPECIFY_FEATURE_DIRECTORY`**:
   1. If the user explicitly provided `SPECIFY_FEATURE_DIRECTORY` (e.g., via environment variable, argument, or configuration), use it as-is
   2. Otherwise, auto-generate it under `.agents/project-harness/specs/`:
      - Check `.agents/project-harness/runtime/init-options.json` for `feature_numbering` (preferred) or `branch_numbering` (deprecated, migration only — will be removed in a future release)
      - If `"timestamp"`: prefix is `YYYYMMDD-HHMMSS` (current timestamp)
      - If `"sequential"` or absent: prefix is `NNN` (next available 3-digit number after scanning existing directories in `.agents/project-harness/specs/`)
      - Construct the directory name: `<prefix>-<short-name>` (e.g., `003-user-auth` or `20260319-143022-user-auth`)
      - Set `SPECIFY_FEATURE_DIRECTORY` to `.agents/project-harness/specs/<directory-name>`
   ...
   **Create the directory and spec file**:
   ...
   - Persist the resolved path to `.agents/project-harness/runtime/feature.json`:
     ```json
     {
       "feature_directory": "<resolved feature dir>"
     }
     ```
     Write the actual resolved directory path value (for example, `.agents/project-harness/specs/003-user-auth`), not the literal string `SPECIFY_FEATURE_DIRECTORY`.
     This allows downstream commands (`$ph-plan`, `$ph-tasks`, etc.) to locate the feature directory without relying on git branch name conventions.

   **IMPORTANT**:
   - You must only create one feature per `$ph-specify` invocation
   - The spec directory name and the git branch name are independent — they may be the same but that is the user's choice
   - The spec directory and file are always created by this command, never by the hook

4. Load the resolved active `spec-template` file to understand required sections.

5. **IF EXISTS**: Load `.agents/project-harness/constitution.md` for project principles and governance constraints.
...
8. **Specification Quality Validation**: After writing the initial spec, validate it against quality criteria:
   ...
   a. **Create Spec Quality Checklist**: Generate a checklist file at `SPECIFY_FEATURE_DIRECTORY/checklists/requirements.md` using the checklist template structure with these validation items:
      ...
      **Feature**: [Link to spec.md]
      ...
      - Items marked incomplete require spec updates before `$ph-clarify` or `$ph-plan`
      ```
...
- Readiness for the next phase (`$ph-clarify` or `$ph-plan`)
