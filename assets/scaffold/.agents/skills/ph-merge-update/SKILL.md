...
升级交付的是可直接工作的项目，不是旧版文件归档后再安装一套空模板。开始任何覆盖、搬迁或退役操作前，完整盘点旧 PH 的技能及其附属资源、配置、入口、运行脚本、文档、记忆、规格、意图和升级记录；包括项目定制、未提交与未跟踪内容。共享目录里无关的用户文件不接管。

每份材料有两项独立验收：

1. **原件保全**：在 `.agents/project-harness/archive/upgrades/<本次批次>/` 保存可按原路径恢复的原件，记录类型、校验值和归属依据。归档中的旧技能和入口不得作为活动指令被发现；链接只登记、不跟随到仓外。
2. **有效内容移植**：指出仍有效的规则、配置、项目事实、需求、任务状态、记忆和技能定制在新终态中的具体位置及验证证据。仅归档的材料必须说明为什么不再需要作为当前内容使用。

需要语义合并时必须阅读旧正文，不能只比较文件名或复制新模板。旧技能中的项目参数和附属资源、旧意图中的原话与验收条件、旧 AGENTS.md 的有效约束，均不能以“原件已归档”代替移植。

新活动内容位于 `.agents/project-harness/`：现行约束在 `constraints/`，项目资料在 `documents/`，活动记忆在 `memory/`，规格在 `specs/`，Spec Kit 运行文件在 `runtime/`。constitution 位于本目录根；在同次升级内完成物化，保留已有项目原则，逐文件引用全部 constraints，并声明每篇何时需要使用。不能留空骨架、待确认使用条件，或要求用户升级后再启动另一个技能补完。

精简 `.agents/AGENTS.md` 前，将需移出的有效规则纳入 constraints 并接入宪法；入口保留读取宪法的要求，不保留技能表和集中调用门禁。每个技能的元数据声明其自己的调用条件。

归档核验、有效内容承接和新路径实际运行全部完成后，才清理旧活动入口并允许 finalize。未解决的归属、语义或内容冲突必须阻断，版本不得提前推进。中断后按同一批次记录续做，不重新定义原件基准，不重复嵌套归档。项目原有历史并不授权发布、提交或修改用户 HOME。
...
stdout JSON 字段：`root` `version` `tag` `commit` `source`。`source` 是固定仓库 URL 字符串（1.x 仍为旧地址，保留兼容含义，不改写、不当错误）。本地已有该 commit 的检查不访问网络。准备成功后把脚本提示转告用户；没登录不拦升级。本会话刚用旧脚本 prepare 时，用发行根补跑 `python3 <release-root>/scripts/ph_release.py support`，不必为了加星再下一遍包。检查 / 同步仍然离线，不重新 prepare，也不为了加星上网。spec-kit 技能与运行时随 PH 发行包分发，安装和升级不再访问官方 Spec Kit。
...
1. **确认**。已初始化才继续。未初始化回到 `ph-init` 的安装 / adopt，不要在本步骤里装新仓。核实目标版本（默认 `latest`）与已有授权，不重复询问已经明确的选择。用户尚未决定是否启动升级时，实际询问“现在开始检查升级差异吗？”，选项为“检查差异，写入前确认范围（推荐）”与“暂不启动升级”；用户已要求升级时直接准备和检查，写入仍按后续流程确认具体范围。提问方式见 [对用户提问](../../project-harness/constraints/harness规范/对用户提问规范.md#把问题写成可回答的决定)，不提供绕过合并检查或必要审批的“直接写入”选项。
...
7. **项目化验收与 verify**。读取发行根 `references/项目化验收.md`，逐篇补齐终态基础树与证据报告，保留有效旧专题。先执行 `ph_speckit.py refresh-navigation`（审阅旧导航、提供来源哈希，先预检再 apply），再执行 `ph_speckit.py verify-content`。空模板、未处理冲突和只归档未承接都不能推进版本。项无 `pending`/`blocked`，必需 Skill 清单与目录实态符合目标，`report.md` 完整。不通过不 finalize。
...
向用户确认升级或冲突时按 [对用户提问](../../project-harness/constraints/harness规范/对用户提问规范.md) 的“把问题写成可回答的决定”发问，不要把项编号念给用户。
...
| id | 做完的样子 |
| --- | --- |
| `docs-sync-skill` | 新必需 Skill `ph-docs-sync` 已从发行根安装到 `.agents/skills/ph-docs-sync/`；`.agents/AGENTS.md` 技能表与 `docs/约束规范/harness规范/文档治理规范.md` §6、`初始化与文档补全.md` 的规则索引已合并指向该 Skill；**本项不自动同步任何业务文档**（README / docs / Wiki 的核验修复由用户另行发起 `ph-docs-sync`）；项目定制与 adapter mode 保留 |
...
| id | 做完的样子 |
| --- | --- |
| `speckit-core-integration` | 十个规格驱动技能 `ph-analyze`、`ph-checklist`、`ph-clarify`、`ph-constitution`、`ph-converge`、`ph-implement`、`ph-plan`、`ph-specify`、`ph-tasks`、`ph-taskstoissues` 已由发行根 `scripts/ph_speckit.py install --apply` 从 PH 随包资产安装（来源由 `speckit.json` 与随包清单记录，安装不拉取官方或生成缓存；统一 `ph-` 前缀，运行时位于 `.agents/project-harness/runtime/`，frontmatter `x-ph-upstream` 与 `.agents/ph.json` 的 `speckit` 节记录来源映射）；`.claude/skills` 镜像同步；已有项目宪法原则保留并迁移到 `.agents/project-harness/constitution.md`。同目录已存在的**同名自定义技能**使本项 `blocked`（保留原件、请用户决定），不覆盖。安装或获取失败时整次升级停止、不推进版本，状态可重试。项目显式规则明确禁止安装上游技能时 `blocked`，交用户裁决 |
...
读 `<release-root>/migrations/1.1.14-to-1.1.15.md`，从更早版本出发仍须读完整链。

| id | 做完的样子 |
| --- | --- |
| `speckit-integrity-gates` | 用新发行根 `ph_speckit.py install` 先规划再 `--apply` 修复缺失的官方运行文件、补全与当前生成结果相同文件的基准；`ph_speckit.py verify` 通过且真实结果登记为本项证据。内容不同且无法证明未修改时停止，不强制覆盖、不伪造基准；用户内容与已有基准保留 |

## 1.2.1 记忆技能项

读 `<release-root>/migrations/1.1.15-to-1.2.1.md`，从更早版本出发仍须读完整链。

| id | 做完的样子 |
| --- | --- |
| `memory-skills` | 三个记忆技能 `ph-memory-ask`、`ph-memory-learning`、`ph-memory-archive` 已从发行根以 scaffold 字节安装（含 evals；Claude 镜像同步）；`ph-memory-ask` 是唯一按回忆意图自动触发的例外门禁，其余两个仅点名触发；`ph-memory-learning` 以记录与资料学习两种输入承接写入（原 `ph-memory-capture` 职责并入，该名不再发行）。双档定位（项目档 `project-harness/memory/`、个人档 `~/.agents/memory/`）与秘密边界已在正文声明。同名目录磁盘版本 ≥ 1.1.14 时本项 `blocked`（更低版本是官方历史副本，由退役项处理后安装）；1.1.14 及以后出现的 `ph-memory-capture` 残留目录按用户内容处理：本项 `blocked`，请用户移除或改名后重跑，不覆盖 |

## 1.2.1 家目录重组项

同样读取 `<release-root>/migrations/1.1.15-to-1.2.1.md`。

| id | 做完的样子 |
| --- | --- |
| `ph-home-restructure` | PH 活动内容统一迁入 `.agents/project-harness/`：约束在 `constraints/`、资料在 `documents/`、记忆在 `memory/`、运行时在 `runtime/`（原根 `.specify/` 退役）、规格在 `specs/`、`.agents/archived/` 并入 `archive/legacy-backup/`；升级前全部旧 PH 原件按哈希进 `archive/upgrades/<批次>/` 并有 `inventory.json` 与逐文件迁移决定；`ph.json`/`ph.schema.json` 按 1.2.1 路径 re-key、基准重记；`.agents/AGENTS.md` 精简为读取宪法的入口（技能表与集中调用门禁移除，调用条件由各技能 description 声明）。目标路径被无关文件占用、符号链接或项目规则与终态相反时 `blocked` |

## 1.2.1 宪法物化项

| id | 做完的样子 |
| --- | --- |
| `constitution-materialization` | `project-harness/constitution.md` 在本次升级内完成物化：已有原则原样保留，空骨架由物化宪法替代（真实项目名、零占位符）；`constraints/` 每个文件（模板除外）都被单独引用且带两行制使用时机声明，索引集合与磁盘集合一致、链接可解析；无法无损合并或使用条件无法确定且影响语义时 `blocked`，不以"待确认"交付 |

## 1.2.1 意图退役项

| id | 做完的样子 |
| --- | --- |
| `intent-retirement` | 旧意图内容先完成迁移核验（不把 `specs/intent-<id>/spec.md` 的存在当作完整证明）：仍有效的需求、原话、验收结论与附件进入 `project-harness/specs/`，有效待办保留活状态；核验通过后 `docs/意图/` 整树、历史索引、`specs/.ph-intent-ledger.json` 与 `select-intent-spec` 退役，原件在升级归档中保全；从 1.1.13 及更早升级的项目由本项直接产出 1.2.1 终态，不生成又删除 `intent-to-spec` 中间产物 |
