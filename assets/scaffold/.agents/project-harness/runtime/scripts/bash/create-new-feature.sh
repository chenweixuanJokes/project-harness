...
# Resolve repository root using common.sh functions which prioritize .agents/project-harness/runtime
...
SPECS_DIR="$REPO_ROOT/.agents/project-harness/specs"
...
if [ "$DRY_RUN" != true ]; then
    ...
    # Persist to .agents/project-harness/runtime/feature.json so downstream commands can find the feature
