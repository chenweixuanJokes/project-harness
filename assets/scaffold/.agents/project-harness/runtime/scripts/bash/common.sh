...
# Find repository root by searching upward for .agents/project-harness/runtime directory
...
find_specify_root() {
    ...
    local prev_dir=""
    while true; do
        if [ -d "$dir/.agents/project-harness/runtime" ]; then
            echo "$dir"
            return 0
        fi
...
# *contains* .agents/project-harness/runtime/), for non-interactive / CI use — e.g. running a Spec Kit
...
# must exist and contain .agents/project-harness/runtime/, with no silent fallback to cwd or the
...
resolve_specify_init_dir() {
    ...
    if [[ ! -d "$init_root/.agents/project-harness/runtime" ]]; then
        echo "ERROR: SPECIFY_INIT_DIR is not a Spec Kit project (no .agents/project-harness/runtime/ directory): $init_root" >&2
        return 1
    fi
    printf '%s\n' "$init_root"
}

# Get repository root, prioritizing .agents/project-harness/runtime directory
...
get_repo_root() {
    ...
    # First, look for .agents/project-harness/runtime directory (spec-kit's own marker)
    local specify_root
...
# the git extension) or implicitly via .agents/project-harness/runtime/feature.json.
get_current_branch() {
...
# Safely read .agents/project-harness/runtime/feature.json's "feature_directory" value.
...
read_feature_json_feature_directory() {
    local repo_root="$1"
    local fj="$repo_root/.agents/project-harness/runtime/feature.json"
...
# Persist a feature_directory value to .agents/project-harness/runtime/feature.json.
...
_persist_feature_json() {
    ...
    # Ensure .agents/project-harness/runtime/ directory exists
    mkdir -p "$repo_root/.agents/project-harness/runtime"
...
get_feature_paths() {
    ...
    # --no-persist so pure path resolution never writes .agents/project-harness/runtime/feature.json,
    ...
    #   2. .agents/project-harness/runtime/feature.json "feature_directory" key (persisted by specify command)
    ...
    if [[ -n "${SPECIFY_FEATURE_DIRECTORY:-}" ]]; then
    ...
    elif [[ -f "$repo_root/.agents/project-harness/runtime/feature.json" ]]; then
        local _fd
        ...
        if [[ -n "$_fd" ]]; then
            ...
            echo "ERROR: Feature directory not found. Set SPECIFY_FEATURE_DIRECTORY or ensure .agents/project-harness/runtime/feature.json contains feature_directory." >&2
            return 1
        fi
    else
        echo "ERROR: Feature directory not found. Set SPECIFY_FEATURE_DIRECTORY or run the specify command to create .agents/project-harness/runtime/feature.json." >&2
        return 1
    fi
...
get_invoke_separator() {
    ...
    local integration_json="$repo_root/.agents/project-harness/runtime/integration.json"
    local separator="."
    local parsed=0
...
#   1. .agents/project-harness/runtime/templates/overrides/
#   2. .agents/project-harness/runtime/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .agents/project-harness/runtime/extensions/<ext-id>/templates/
#   4. .agents/project-harness/runtime/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.agents/project-harness/runtime/templates"
    ...
    local presets_dir="$repo_root/.agents/project-harness/runtime/presets"
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.agents/project-harness/runtime/extensions"
    if [ -d "$ext_dir" ]; then
